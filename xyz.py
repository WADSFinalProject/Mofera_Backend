...
@router.get("/get_checkpoint_datas", dependencies=[Depends(role_access(RoleEnum.xyz))])
def get_checkpoint_datas(db:db_dependecy, p: int = 0):

    db_checkpoint = crud.get_checkpoints(db=db, limit=50,)
    return db_checkpoint

@router.get("/get_checkpoint_summary", dependencies=[Depends(role_access(RoleEnum.xyz))])
def get_checkpoint_summary(db:db_dependecy):

    db_checkpoint = crud.get_checkpoint_summary(db=db)
    return db_checkpoint

@router.get("/quick_get_checkpoint_stats", dependencies=[Depends(role_access(RoleEnum.xyz))])
def quick_checkpoint_statistics(db:db_dependecy, interval:str, date:date = date.today(), slice:int = 6):
    label = list()
    data = list()
    if interval == "daily":
        for offset in range(slice):
            offset_date = date - timedelta(days=offset)
            label.append(offset_date.strftime("%d/%m/%Y"))
            data.append(len(crud.get_checkpoints(db=db, limit=0, year=offset_date.year, month=offset_date.month, day=offset_date.day)))
    
    elif interval == "weekly":
        for offset in range(slice):
            offset_date = date - timedelta(days=date.weekday()+1+(offset-1)*7)
            label.append(offset_date.strftime("%d/%m/%Y"))
            data.append(len(crud.get_checkpoints(db=db, limit=0, year=offset_date.year, month=offset_date.month, day=offset_date.day, filter="w")))
        ...
        for offset in range(slice):
            ...
            data.append(len(crud.get_checkpoints(db=db, limit=0, year=offset_date.year, month=offset_date.month)))
...
def get_shipping_summary(db:db_dependecy, centra_id: int=0):
    ...
    db_checkpoint = crud.get_shipment_summary(db=db, centra_id=centra_id)
    return db_checkpoint
